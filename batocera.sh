...
for each in "${roms[@]}"; do 
  ...
  if [[ -z "${seen[${rom[0]}]}" ]]; then
    ...
    if [ ! -d "/userdata/thumbs/${rom[2]}" ]; then
       wget "https://github.com/WizzardSK/${rom2}/archive/refs/heads/master.zip" -O /tmp/"${rom2}".zip && unzip /tmp/"${rom2}".zip -d /userdata/thumbs/"${rom[2]}" && rm /tmp/"${rom2}".zip && mv /userdata/thumbs/"${rom[2]}"/"${rom[2]}"-main/* /userdata/thumbs/"${rom[2]}" 2>&1 | tee -a /userdata/system/logs/git.log
    fi
  fi  
  ...
  if ! grep -Fxq "<gameList>" /userdata/roms/"${rom[0]}"/gamelist.xml > /dev/null 2>&1; then
    ls /userdata/roms/"${rom[0]}"/"${rom3}" | while read line; do
      ...
      if grep -iqE 'pal|europe' <<< "$line"; then
